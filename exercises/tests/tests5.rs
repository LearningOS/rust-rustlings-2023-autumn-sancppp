...
unsafe fn modify_by_address(address: usize) {
    // SAFETY: The `address` parameter is assumed to be a valid memory address
    // pointing to a mutable `u32` value. It is the responsibility of the caller
    // to ensure the validity of the address and that it points to a valid `u32`.
    // This function modifies the value at the given address.
    unsafe {
